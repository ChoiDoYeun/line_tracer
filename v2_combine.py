...
def avoid_obstacle_and_return():
    ...
    if ld > rd:
        ...
        while ld > 0.4:
            ...
            with lidar_lock:
                ld = left_distance
            time.sleep(0.01)
        rotation_time = time.time() - rotation_start_time
    else:
        print("우측 회피 시작")
        while rd > 0.4:
    ...
    # 8. 좌우측 거리 측정하여 좁은 쪽의 거리가 1m 이상이 될 때까지 회전
